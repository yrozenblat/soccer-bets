...
def main(path: str, output_path: str | None = None) -> None:
    ...
    print(f"  t_lower={result['t_lower']:.3f}  t_upper={result['t_upper']:.3f}")
    ...
    for cat in ("Dominant", "Contested", "Open"):
        ...
        print(f"  {cat:10s}: {count:2d} matches  canonical={score_str}  pts={pts}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python optimize.py <path-to-csv> [output-model.json]")
        sys.exit(1)
